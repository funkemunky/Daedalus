...
public class DebugArgument extends FunkeArgument {
    ...
    public DebugArgument(FunkeCommand parent, String name, String display, String description, String... permission) {
        ...
        Daedalus.getInstance().getCheckManager().getChecks().forEach(check -> checks.add(check.getName().replaceAll(" ", "_")));
            ...
            if (data != null) {
                if(args.length > 1) {
                    if (args[1].equalsIgnoreCase("box")) {
                        ...
                        Check check = Daedalus.getInstance().getCheckManager().getCheck(args[1].replaceAll("_", " "));
