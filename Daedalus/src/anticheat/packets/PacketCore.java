    ...
    public PacketCore(Daedalus Daedalus) {
        ...
        ProtocolLibrary.getProtocolManager().addPacketListener(new PacketAdapter(Daedalus, ENTITY_PACKETS){
        	 ...
        	 public void onPacketSending(PacketEvent event) {
                 ...
                 if (e instanceof LivingEntity && enabled.contains((Object)e.getType()) && packet.getWatchableCollectionModifier().read(0) != null && e.getUniqueId() != event.getPlayer().getUniqueId()) {
                     packet = packet.deepClone();
                     event.setPacket(packet);
                     if(event.getPacket().getType() == PacketType.Play.Server.ENTITY_METADATA) {
                    	 WrappedDataWatcher watcher = new WrappedDataWatcher((List)packet.getWatchableCollectionModifier().read(0));
